...
public class App {
    ...
    public String listMessages() {
        ...
        for (Messaggio message : selected) {
            ...
            for (Indirizzo indirizzo : (List<Indirizzo>) it.next().valore())
    ...
    public String readMessage(int n) {
        ...
        for (Intestazione intestazione : (Iterable<Intestazione>) messaggio::intestazioni) {
            ...
            if (intestazione instanceof Destinatario destinatario)
                values.add(
                        destinatario.valore().stream().map(Indirizzo::toString).reduce((a, b) -> a + "\n" + b).get());
            else
    ...
    public Object deleteMessage(int n) {
        return selected.removeMessage(n);
    }
    ...
    public void compose(String string) {
        ...
        try (Scanner sc = new Scanner(string)) {
            ...
            intestazioni.add(new Data(DateEncoding.decode(ASCIICharSequence.of(sc.nextLine()))));
            StringJoiner sj = new StringJoiner("\n");
            String corpo;

            ContentType text = new ContentType("text/plain", "charset=\"us-ascii\"");
            while (!(corpo = sc.nextLine()).equals("."))
                sj.add(corpo);
            if (!sj.toString().isBlank())
                parti.add(new Parte(text, sj.toString()));
            ...
            while (!(corpo = sc.nextLine()).equals("."))
                sj.add(corpo);
            if (!sj.toString().isBlank())
                parti.add(new Parte(html, sj.toString()));
        }
